...
This report analyses loan default trends using Lending Club data
covering one million anonymised peer-to-peer loans. The analysis
addresses the key questions posed by the Director of the Credit
Institute in Texas, including identifying default risk drivers,
evaluating the grading system, recommending a DTI hard-cap, and testing
four institutional heuristics against the data.
...
\begin{itemize}
\item
  Two binary outcome variables were created from \texttt{loan\_status}.
  \texttt{Default\_Flag} takes the value 1 for loans charged off or
  defaulted, 0 for fully paid, and NA for current loans. These are
  excluded from default rate calculations as their outcome is not yet
  known. \texttt{Collections\_Flag} is derived from
  \texttt{collections\_12\_mths\_ex\_med} and flags borrowers who have
  missed payments but have not yet fully defaulted, consistent with the
  director's definition of partial default.
\item
...
\item
  ...
  (highest risk).
\item
  The dataset contains no direct age variable so Credit history length,
  derived from \texttt{earliest\_cr\_line}, is used as a proxy since a
  longer history is likely linked to an older borrower.
\item
  Continuous variables used in the analysis were Winsorized at the 1st
  and 99th percentiles to prevent extreme values from distorting
  results.
\end{itemize}

\subsection{Key Risk Drivers}\label{key-risk-drivers}

\subsection{Default Rate by Credit
Grade}\label{default-rate-by-credit-grade}

\pandocbounded{\includegraphics[keepaspectratio,alt={Default rates rise monotonically from Grade A to G, confirming the grading system captures genuine credit risk.}]{Question3_files/Question3_files/figure-latex/plot-grade-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Loan purpose varies substantially in associated default risk.}]{Question3_files/Question3_files/figure-latex/plot-purpose-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The director's heuristic receives only partial support.}]{Question3_files/Question3_files/figure-latex/plot-heuristic-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Default rates vary across states but Texas sits close to the national average.}]{Question3_files/Question3_files/figure-latex/plot-state-1.pdf}}
...
culture. Texas sits close to the average when looking at the bigger
picture.
...
\pandocbounded{\includegraphics[keepaspectratio]{Question3_files/Question3_files/figure-latex/plot-grade-age-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Question3_files/Question3_files/figure-latex/plot-int-rate-1.pdf}}
